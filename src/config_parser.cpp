            ...
            if (jsonBody.has("email_credentials_file")) {
                ...
                if (!credFile.is_open()) {
                    if (!config.email_recipients.empty()) {
                        std::cerr << "Warning: Could not open email credentials file at "
                                  << credPath
                ...
                if (credFile.is_open()) {
                    ...
                    if (credJson) {
                        ...
                        if (credJson.has("password")) {
                            ...
                            std::cerr << "Warning: 'password' field missing in credentials file "
                                      << credPath << "\n";
                        }
                    } else {
            ...
            if (jsonBody.has("gmail_calendar_credentials_file")) {
                ...
                if (!credFile.is_open()) {
                    std::cerr << "Warning: Could not open calendar credentials file at " << credPath
                              << ". Checking /secrets_calendar/calendar_credentials.json as "
                                 "fallback...\n";
